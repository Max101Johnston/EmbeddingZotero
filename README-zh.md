...
- 文献移入回收站、永久删除或合并去重后，清理对应的向量、索引状态和内容缓存；启动后及定期检查会补清理遗漏的条目。
...
索引以“我的文库”中仍存在的条目为准。从某个分类中移出文献并不等于从文库删除，因此索引会保留。清理旧索引不调用嵌入 API；新文献仍须成功嵌入后才会有向量。SQLite 删除记录后可能复用空闲页面，数据库文件大小不一定马上变小。
