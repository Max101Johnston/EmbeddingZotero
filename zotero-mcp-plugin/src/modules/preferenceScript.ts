...
import { getIndexConcurrency, INDEX_CONCURRENCY_PREF, parseIndexConcurrency } from "./semantic/indexConcurrency";
...
function bindEmbeddingSettings(doc: Document) {
  ...
  // Use the same model capability check as the request builder so a dimension
  // shown as editable here is also sent to the embedding API.
  const supportsCustomDimensions = (model: string) => {
    const { EmbeddingService } = require("./semantic/embeddingService");
    return EmbeddingService.supportsCustomDimensions(model);
  };
    ...
    try {
      ...
      if (dims >= 0) {
        ...
        try {
          ...
          await vectorStore.initialize();
          const stats = await vectorStore.getStats();
          const counts = stats.dimensionCounts || [];
          storedDistribution = counts.map((row: any) => String(row.dimensions)).join(', ');
          hasStoredMismatch = counts.some((row: any) => row.dimensions !== dims);
        ...
        if (hasStoredVectors && hasStoredMismatch) {
          // Dimension mismatch with existing index - warn but don't auto-update
          testResult.textContent = `${successMsg} - ⚠️ ${getString("pref-embedding-dimension-mismatch" as any) || "Some stored vectors have different dimensions. Use selective re-embedding."} (${storedDistribution})`;
          testResult.style.color = "var(--color-warn)";

          // Keep the observed result without changing the requested dimensions.
          const { getEmbeddingService } = require("./semantic/embeddingService");
          getEmbeddingService().setDetectedDimensions(dims);
        } else {
          ...
          if (dims > 0) {
            ...
            try {
              ...
              embeddingService.setDetectedDimensions(dims);
            } catch (e) {
              // Ignore
            }
          }
...
/** Bind the validated item indexing concurrency preference. */
function bindIndexConcurrency(doc: Document) {
  const input = doc?.querySelector(
    `#zotero-prefpane-${config.addonRef}-semantic-index-concurrency`,
  ) as HTMLInputElement;
  if (!input) return;

  input.value = String(getIndexConcurrency());
  input.addEventListener("change", () => {
    const concurrency = parseIndexConcurrency(input.value);
    if (concurrency === null) {
      input.value = String(getIndexConcurrency());
      addon.data.prefs!.window.alert(
        getString("pref-semantic-index-concurrency-invalid" as any) ||
        "请输入 1 到 1000 的整数。 / Enter an integer from 1 to 1000.",
      );
      return;
    }
    Zotero.Prefs.set(INDEX_CONCURRENCY_PREF, concurrency, true);
  });
}
...
function bindSemanticStatsSettings(doc: Document) {
  ...
  const libraryItemsEl = doc?.querySelector("#semantic-stats-library-items") as HTMLElement;
  const regularItemsEl = doc?.querySelector("#semantic-stats-regular-items") as HTMLElement;
  const standaloneItemsEl = doc?.querySelector("#semantic-stats-standalone-items") as HTMLElement;
  const unindexedItemsEl = doc?.querySelector("#semantic-stats-unindexed-items") as HTMLElement;
  const nonCurrentItemsEl = doc?.querySelector("#semantic-stats-non-current-items") as HTMLElement;
  const rawItemsEl = doc?.querySelector("#semantic-stats-raw-items") as HTMLElement;
  ...
  const detectDimensionsButton = doc?.querySelector("#detect-index-dimensions-button") as HTMLButtonElement;
  const reembedButton = doc?.querySelector("#reembed-mismatched-button") as HTMLButtonElement;
  const dimensionReportEl = doc?.querySelector("#semantic-dimension-report") as HTMLElement;
  ...
  function showDimensionAudit(audit: {
    currentDimensions: number;
    dimensionCounts: Array<{ dimensions: number; vectors: number; items: number }>;
    mismatchedItems: number;
  }) {
    if (!dimensionReportEl) return;
    const distribution = audit.dimensionCounts.length
      ? audit.dimensionCounts.map(row => `${row.dimensions}D: ${row.vectors}/${row.items}`).join(' · ')
      : '-';
    dimensionReportEl.textContent =
      `${getString("pref-semantic-dimension-current" as any) || "Current model"}: ${audit.currentDimensions}D · ` +
      `${getString("pref-semantic-dimension-distribution" as any) || "Stored vectors/items"}: ${distribution} · ` +
      `${getString("pref-semantic-dimension-mismatched" as any) || "Items needing repair"}: ${audit.mismatchedItems}`;
    dimensionReportEl.style.color = audit.mismatchedItems ? 'var(--color-warn)' : 'var(--color-ok)';
  }

  detectDimensionsButton?.addEventListener("click", async () => {
    if (isIndexing) return;
    detectDimensionsButton.disabled = true;
    try {
      const { getSemanticSearchService } = require("./semantic");
      showDimensionAudit(await getSemanticSearchService().auditIndexDimensions());
    } catch (error) {
      showMessage(`${getString("pref-semantic-index-error" as any) || "Index error"}: ${error}`, "error");
    } finally {
      detectDimensionsButton.disabled = false;
    }
  });

  reembedButton?.addEventListener("click", () => {
    if (isIndexing) return;
    const confirmMsg = getString("pref-semantic-dimension-confirm-reembed" as any) ||
      "Re-embed only items with mismatched dimensions? This calls the embedding API.";
    if (addon.data.prefs!.window.confirm(confirmMsg)) startIndexing(false, true);
  });
  ...
  resumeButton?.addEventListener("click", async () => {
    try {
      ...
      if (!isIndexing && (progress.status === 'paused' || progress.status === 'error')) {
        ...
        const resumeResult = progress.mode === 'repair'
          ? await semanticService.reembedMismatchedItems(onResumeProgress)
          : await semanticService.buildIndex({ rebuild: false, onProgress: onResumeProgress });
  ...
  async function startIndexing(rebuild: boolean, repairDimensions = false) {
    if (isIndexing) return;
    isIndexing = true;
    ...
    try {
      ...
      const onProgress = (progress: any) => updateProgress(progress);
      const result = repairDimensions
        ? await semanticService.reembedMismatchedItems(onProgress)
        : await semanticService.buildIndex({ rebuild, onProgress });
      ...
      if (result.status === 'busy') {
        ...
        if (result.total === 0) {
          showMessage(
            repairDimensions
              ? (getString("pref-semantic-dimension-no-live-items" as any) || "No live items need re-embedding")
              : (getString("pref-semantic-index-no-items" as any) || "No items need indexing"),
            "info"
          );
        } else {
          ...
          if (failedItems.length > 0) {
            ...
            const completed = repairDimensions
              ? (getString("pref-semantic-dimension-reembed-completed" as any) || "Re-embedding completed")
              : (getString("pref-semantic-index-completed" as any) || "Indexing completed");
            showMessage(`${completed} (${result.processed}/${result.total})`, "success");
          }
        }
      ...
      // Reload stats
      loadSemanticStats();
      if (repairDimensions && result.status === 'completed') {
        try {
          const audit = await semanticService.auditIndexDimensions();
          showDimensionAudit(audit);
          if (audit.mismatchedItems > 0) {
            showMessage(
              `${getString("pref-semantic-dimension-reembed-incomplete" as any) || "Some stored items still have mismatched dimensions"}: ${audit.mismatchedItems}`,
              "warning",
            );
          }
        } catch (auditError) {
          ztoolkit.log(`[PreferenceScript] Post-repair dimension audit failed: ${auditError}`, "warn");
        }
      }
  ...
  function updateControlButtons(status: 'idle' | 'indexing' | 'paused') {
    ...
    if (detectDimensionsButton) detectDimensionsButton.style.display = status === 'idle' ? '' : 'none';
    if (reembedButton) reembedButton.style.display = status === 'idle' ? '' : 'none';
  ...
  async function loadSemanticStats(silent = false) {
    ...
    try {
      ...
      // Update UI
      const library = stats.libraryStats;
      if (totalItemsEl) totalItemsEl.textContent = library ? String(library.indexedRegularItems) : '-';
      if (libraryItemsEl) libraryItemsEl.textContent = library ? String(library.topLevelItems) : '-';
      if (regularItemsEl) regularItemsEl.textContent = library ? String(library.regularItems) : '-';
      if (standaloneItemsEl) standaloneItemsEl.textContent = library ? String(library.standaloneAttachments + library.standaloneNotes) : '-';
      if (unindexedItemsEl) unindexedItemsEl.textContent = library ? String(library.unindexedRegularItems) : '-';
      if (nonCurrentItemsEl) nonCurrentItemsEl.textContent = library ? String(library.nonCurrentIndexItems) : '-';
      if (rawItemsEl) rawItemsEl.textContent = String(stats.indexStats.totalItems);
      ...
      if (dimensionsEl) {
        const counts = stats.indexStats.dimensionCounts || [];
        dimensionsEl.textContent = counts.length
          ? counts.map((row: any) => `${row.dimensions}D: ${row.vectors}/${row.items}`).join(' · ')
          : '-';
        dimensionsEl.style.color = counts.length > 1 ? 'var(--color-warn)' : 'var(--color-default)';
      }
      if (int8StatusEl) {
      ...
      if (!silent) {
        ztoolkit.log(`[PreferenceScript] Loaded semantic stats: ${library?.indexedRegularItems ?? '-'} current indexed items, ${stats.indexStats.totalItems} stored item keys, ${stats.indexStats.totalVectors} vectors`);
      }
