...
const PREF_DETECTED_CONFIG = 'extensions.zotero.zotero-mcp-plugin.embedding.detectedConfig';
...
export class EmbeddingService {
  ...
  private loadConfigFromPrefs(): void {
    try {
      ...
      const detectedConfig = Zotero.Prefs.get(PREF_DETECTED_CONFIG, true);
      ...
      // A dimension observed with another model, endpoint, or output setting
      // must not override the current configuration after a restart.
      const parsedDetectedDims = parseInt(String(detectedDims), 10);
      this.detectedDimensions = detectedConfig === this.getConfigSignature() &&
        Number.isInteger(parsedDetectedDims) && parsedDetectedDims > 0
        ? parsedDetectedDims : null;
      if (timeoutSeconds) {
  ...
  private saveDetectedDimensions(dims: number): void {
    try {
      ...
      Zotero.Prefs.set(PREF_DETECTED_CONFIG, this.getConfigSignature(), true);
  ...
  /** Record dimensions observed by either indexing or the settings test. */
  setDetectedDimensions(dims: number): void {
    if (!Number.isInteger(dims) || dims <= 0) return;
    this.detectedDimensions = dims;
    this.saveDetectedDimensions(dims);
  }
  ...
  getActualDimensions(): number | null {
    return this.detectedDimensions ||
      (this.supportsCustomDimensions() ? this.config.dimensions || null : null);
  }
   ...
   */
  static supportsCustomDimensions(modelName: string): boolean {
    ...
    if (model.includes('qwen3-embedding') || model.includes('qwen3.7-text-embedding') ||
        model.includes('embeddinggemma') || model.includes('nomic-embed')) return true;
    return false;
  }

  supportsCustomDimensions(): boolean {
    return EmbeddingService.supportsCustomDimensions(this.config.model);
  }
  ...
  clearDetectedDimensions(): void {
    ...
    try {
      ...
      Zotero.Prefs.clear(PREF_DETECTED_CONFIG, true);
  ...
  updateConfig(newConfig: Partial<EmbeddingConfig>): void {
    const previousSignature = this.getConfigSignature();
    this.config = { ...this.config, ...newConfig };
    if (this.getConfigSignature() !== previousSignature) {
      this.clearDetectedDimensions();
      ...
      try {
        ...
        if (embeddings.length > 0 && embeddings[0].length > 0) {
          ...
          if (this.detectedDimensions !== actualDims) {
            this.setDetectedDimensions(actualDims);
