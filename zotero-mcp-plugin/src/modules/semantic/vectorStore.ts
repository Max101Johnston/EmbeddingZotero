...
export class VectorStore {
  ...
  /** Replace one item's vectors and status in one transaction, preserving the old index on failure. */
  async replaceItemVectors(
    itemKey: string,
    ...
    if (!records.length || records.some(record => record.itemKey !== itemKey)) {
      throw new Error('Replacement requires vectors for exactly one item');
    }
    await this.ensureInitialized();
    await this.db.executeTransaction(async () => {
      await this.db.queryAsync(`DELETE FROM embeddings WHERE item_key = ?`, [itemKey]);
      await this.db.queryAsync(`DELETE FROM vectors_f32 WHERE item_key = ?`, [itemKey]);
      for (const record of records) {
        const quantized = this.quantizeWithNorm(record.vector);
        await this.db.queryAsync(`INSERT OR REPLACE INTO embeddings (item_key, chunk_id, vector, language, chunk_text, dimensions, vector_int8, vector_scale, vector_norm) VALUES (?, ?, x'', ?, ?, ?, ?, ?, ?)`, [
          record.itemKey, record.chunkId, record.language, record.chunkText || '',
          record.vector.length, this.int8ArrayToBase64(quantized.int8Data), quantized.scale, quantized.norm,
        ]);
        await this.db.queryAsync(`INSERT OR REPLACE INTO vectors_f32 (item_key, chunk_id, vector) VALUES (?, ?, ?)`, [
          record.itemKey, record.chunkId, this.float32ArrayToBuffer(record.vector),
        ]);
      }
      await this.db.queryAsync(`INSERT OR REPLACE INTO index_status (item_key, indexed_at, version, chunk_count, content_hash, item_modified, attachment_modified) VALUES (?, strftime('%s', 'now'), 1, ?, ?, ?, ?)`, [
        itemKey, records.length, contentHash, itemModified || null, attachmentModified || null,
      ]);
    });
    for (const key of this.vectorCache.keys()) {
      if (key.startsWith(`${itemKey}_`)) this.vectorCache.delete(key);
    }
    for (const record of records) this.updateCache(`${itemKey}_${record.chunkId}`, record.vector);
  }

  /** Read-only scan of every stored vector dimension, including mixed items. */
  async getDimensionCounts(): Promise<Array<{ dimensions: number; vectors: number; items: number }>> {
    await this.ensureInitialized();
    const rows = await this.db.queryAsync(`SELECT dimensions, COUNT(*) AS vectors, COUNT(DISTINCT item_key) AS items FROM embeddings GROUP BY dimensions ORDER BY dimensions`);
    return (rows || []).map((row: any) => ({
      dimensions: Number(row.dimensions), vectors: Number(row.vectors), items: Number(row.items),
    }));
  }

  async getItemKeysWithOtherDimensions(dimensions: number): Promise<string[]> {
    await this.ensureInitialized();
    const rows = await this.db.queryAsync(`SELECT DISTINCT item_key FROM embeddings WHERE dimensions <> ?`, [dimensions]);
    return (rows || []).map((row: any) => row.item_key);
  }
  ...
  async search(
    ...
    // Mixed historical indexes are searchable for the matching dimension.
    conditions.push('dimensions = ?');
    params.push(queryVector.length);
  ...
  /** Keys that actually have vectors, excluding status-only and failed items. */
  async getVectorItemKeys(): Promise<Set<string>> {
    await this.ensureInitialized();
    const rows = await this.db.queryAsync(`SELECT DISTINCT item_key FROM embeddings`);
    return new Set((rows || []).map((row: any) => row.item_key));
  }
  ...
  async getStats(): Promise<VectorStoreStats> {
    ...
    const dimensionCounts = await this.getDimensionCounts();
    const storedDimensions = dimensionCounts.length === 1 ? dimensionCounts[0].dimensions : undefined;
